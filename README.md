...
```
employee-compensation-service/
├── function_app.py              # all HTTP-triggered functions
...
## Quickstart — set up and run from scratch (Windows / PowerShell)

Run these in order in a fresh PowerShell terminal, from the folder where you want the project to live.

```powershell
# 1. Clone the repo
git clone https://github.com/<your-username>/employee-compensation-service.git
...
# 4. Create your local secrets file from the example
cp local.settings.json.example local.settings.json
# Now open local.settings.json and fill in SQL_CONNECTION_STRING
# (Portal → your database → Connection strings → ODBC tab)
...
Functions will be available at `http://localhost:7071/api/...` (locally, function-key auth is not enforced, so no `?code=` is needed). Leave this terminal running — open a **second** terminal for testing endpoints.

### One-time Azure setup (before step 1 works end-to-end)

These only need doing once, via the Azure Portal (no CLI required):

1. **Prerequisites**
   - Python 3.11
   - [Azure Functions Core Tools v4](https://learn.microsoft.com/azure/azure-functions/functions-run-local) (only used to run `func start` locally)
...
2. **Create the database (Portal)**
   - Portal → "SQL databases" → Create → new database `EmployeeCompensationDB`, new server, Serverless/Gen5/1 vCore compute tier, minimum storage.
...
3. **Deploying to Azure (optional, Portal)**
   - Portal → Create a resource → **Function App** → Runtime stack: Python 3.11, Region: same as your DB.
   - **Deployment Center** → connect your GitHub repo (or use the VS Code Azure Functions extension "Deploy" button).
...
Standard workflow for committing and pushing changes after the initial setup:

```powershell
# Check what's changed
git status

# Stage changes (all files, or name specific ones)
git add .
...
If this is the **first ever push** from a freshly cloned/initialized local repo:

```powershell
git init                      # only if the folder isn't a git repo yet
git remote add origin https://github.com/<your-username>/employee-compensation-service.git
git add .
...
If `git push` is rejected because the remote has commits you don't have locally (e.g. GitHub auto-created a README when you made the repo):

```powershell
git pull --allow-unrelated-histories
# resolve any merge conflict markers (<<<<<<<, =======, >>>>>>>) by hand in the affected files
git add .
...
| `Client with IP address 'X.X.X.X' is not allowed to access the server` — recurring with a **different IP almost every request** | Mobile/ISP CGNAT rotating the public IP per-connection (common on Indian mobile data) | Add a temporary firewall **range** rule (e.g. `152.58.0.0`–`152.59.255.255`) covering the pool instead of single IPs; **delete the range rule when done for the day**. Prefer wifi over mobile data when possible. |
| PowerShell `curl -X POST ... -H ... -d ...` throws `ParameterBindingException` / `-H not recognized` | PowerShell's `curl` = `Invoke-WebRequest`, doesn't support curl flags; `\` line continuation is bash-only | Use `curl.exe` explicitly, or switch to `Invoke-RestMethod` |
| `Invoke-RestMethod` / `Invoke-WebRequest` shows only `(404) Not Found` with no detail | These cmdlets swallow the response body on error by default | Wrap in `try/catch` and read `$_.Exception.Response.GetResponseStream()` |
...
| `"Unable to access AzureWebJobsStorage"` on deploy, host reports unhealthy | Function App is missing/misconfigured storage account connection needed by the Functions runtime | In Portal → Function App → **Configuration**, confirm `AzureWebJobsStorage` is set (usually auto-set when the storage account is created alongside the Function App); if it's blank or points to a deleted account, recreate/relink the storage account and redeploy |
